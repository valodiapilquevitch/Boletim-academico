...
<div class="col-xs-6 col-sm-2 bg-green"><p class='text-center'>
	<div class="logado" id="login_id">Olá <?php echo $_SESSION['login']?>
	 <input type="button" value="Fechar" onclick="hide(login_id)" style="height:30px; border:solid 1px white; border-radius:5px;"/>
	</div>
</div>
<div class="col-xs-6 col-sm-1 bg-green"><p class='text-center'> <a href="logout.php"><button type="button" class="btn btn-default navbar-btn" id="logout" >Sair</button></a></p></div>
                </div>
            </div>
...
include("conexao.php");
	...
	WHERE a.username = '" . $_SESSION['login'] . "'") or die(mysql_error());
...
WHERE mdl_user.username= '" . $_SESSION['login'] . "'") or die(mysql_error());
...
WHERE mdl_user.username= '" . $_SESSION['login'] . "'") or die(mysql_error());
    ...
    <script src ="js/jquery.min.js"></script>
		...
		<script>
				function hide(login_id) {
            if (false == $(login_id).is(':visible')) {
                $(login_id).show(250);
            }
            else {
                $(login_id).hide(250);
